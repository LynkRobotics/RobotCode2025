        ...
        return LoggedCommands.sequence("Fast coral score following " + path,
            Commands.deadline(
                Commands.sequence(
                    ...
                    WaitForTowardsNext(),
                    Commands.either(
                        LoggedCommands.proxy(new PIDSwerve(s_Swerve, s_Pose, left ? mirroredFaces.get(face).alignRight : mirroredFaces.get(face).alignLeft, true, true).fastAlign().setTolerance(Constants.Auto.fastPositionTolerance)),
                        LoggedCommands.proxy(new PIDSwerve(s_Swerve, s_Pose, left ? face.alignLeft : face.alignRight, true, true).fastAlign().setTolerance(Constants.Auto.fastPositionTolerance)),
                        this::shouldMirror
                    ),
                    LoggedCommands.proxy(s_Swerve.Stop()),
                    Commands.either(
